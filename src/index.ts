...
function buildStationsPageContent(
...
function buildStationsPaginationRow(page: number, totalPages: number): ActionRowBuilder<ButtonBuilder> {
...
function buildStationsPlayRows(
  stations: RadioStation[],
  page: number
...
client.on('messageCreate', async (message) => {
  ...
  if (lower === '!help') {
    console.log(`[COMMAND] !help | Server: ${guild.name} (${guild.id}) | Channel: ${(message.channel as { name?: string }).name ?? message.channel.id} | User: ${message.author.tag}`);
    const helpText = [
  ...
  if (lower === '!stations' || lower.startsWith('!stations ')) {
    try {
      ...
      console.log(`[COMMAND] !stations (page ${page}) | Server: ${guild.name} (${guild.id}) | Channel: ${(message.channel as { name?: string }).name ?? message.channel.id} | User: ${message.author.tag}`);
      const stations = await loadStations();
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (customId !== STATIONS_PREV_ID && customId !== STATIONS_NEXT_ID) return;
