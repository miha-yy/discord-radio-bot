import { STATIONS_NEXT_ID, STATIONS_PLAY_PREFIX, STATIONS_PREV_ID, } from './constants.js';
import { loadStations } from './radioList.js';
import { buildStationsPageContent, buildStationsPaginationRow, buildStationsPlayRows, } from './stationsUI.js';
import { joinVoiceAndPlayStation } from './voice.js';
export function getStationsButtonType(customId) {
    if (customId.startsWith(STATIONS_PLAY_PREFIX))
        return 'play';
    if (customId === STATIONS_PREV_ID)
        return 'prev';
    if (customId === STATIONS_NEXT_ID)
        return 'next';
    return null;
}
export async function handleStationsPlayButton(interaction, globalIndex) {
...
export async function handleStationsPagination(interaction, direction) {
    const guild = interaction.guild;
